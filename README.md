...
2. 3x BC337 (only 1 needed for monitoring). You could substitute any similar NPN from your parts bin.
3. 1k Resistor
4. 51k Resistor
5. 330R Resistor
6. 3V Zener (optional, holdover from previous iterations - DNP)
7. 39k Resistor
8. 2x10k Resistor
...
10. 1N4007 Diode 
11. Omron GRL-1 12V DC Relay if you want an aux alarm contact (again not a safety or security device).
...
The system will try autodetect which of the two possible keypads is free. If you already have two keypads, it will disable control of the energiser as the energiser only allows for two keypads. If you have no keypads, it will select based on if pin 22 is grounded (keypad 1) or not (keypad 2).
...
1. Some tags are the wrong type, Open to suggestions on what types they should be.
2. The alarm history status and service mode tags are not tracked at this point - I still need to trigger and clear an alarm while the fence is running.
